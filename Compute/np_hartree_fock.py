...
class HartreeFock(BlueprintCircuit):
    ...
    def __init__(
        self,
        num_orbitals: tuple[int, int] | None = None, 
        ...
        """
        Args:
             - num_spatial_orbitals: The number of spatial orbitals. - (removed)
            num_orbitals: This modification replace num_spatial_orbitals, to allow for different alpha and beta numbers of states, 
                TBD
    ...
    @property
    def num_orbitals(self) -> tuple[int, int]:
        """The number of orbitals."""
        return self._num_orbitals

    @num_orbitals.setter
    def num_orbitals(self, n: tuple[int, int]) -> None:
        """Sets the number of orbitals."""
        self._invalidate()
        self._num_orbitals = n
        self._reset_register()
    ...
    def _check_configuration(self, raise_on_failure: bool = True) -> bool:
        ...
        """
        # if self.num_spatial_orbitals is None: ## Modification by HWa
        #     if raise_on_failure:
        ...
        # if any(n > self.num_spatial_orbitals for n in self.num_particles): ## modification by HWa
        #     if raise_on_failure:
        #         raise ValueError(
...
def hartree_fock_bitstring(num_orbitals: tuple[int, int], num_particles: tuple[int, int]) -> list[bool]:
    """Compute the bitstring representing the Hartree-Fock state for the specified system.

    Args:
        num_spatial_orbitals: replaced with num_orbitals
    ...
    """
    # validate the input (no validation for my code, we kampung style)
    num_alpha, num_beta = num_particles
    num_orb_alpha , num_orb_beta = num_orbitals

    bitstr = np.zeros(sum(num_orbitals), bool)
    bitstr[:num_alpha] = True
    bitstr[num_orb_alpha : (num_orb_alpha + num_beta)] = True
